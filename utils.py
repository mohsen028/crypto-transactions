    ...
    if 'transactions' not in st.session_state:
        st.session_state.transactions = pd.DataFrame([
            ...
            },
            {
                ...
                "input_amount": 75.0, "output_amount": 0.003, "rate": 25000.0, "fee": 1.5, "notes": "Bought Bitcoin"
            },
             {
            ...
            },
            {
                ...
                "input_amount": 0.001, "output_amount": 30.0, "rate": 30000.0, "fee": 0.5, "notes": "Sold some BTC for profit"
            },
        ])
...
# ... (Keep TRANSACTION_TYPE_LABELS, PEOPLE, CRYPTOS, CURRENCIES)
...
# --- Price Fetching Logic (No changes needed here) ---
...
# --- NEW: Master Financial Analysis Function ---
def generate_financial_analysis(transactions, prices):
    ...
    toman_stats = toman_tx.groupby('person_name').agg(
        total_toman_paid=('input_amount', 'sum'),
        total_usdt_received=('output_amount', 'sum')
    ).reset_index()
    toman_stats['avg_usdt_cost'] = toman_stats['total_toman_paid'] / toman_stats['total_usdt_received']

    # 2. Floating Portfolio & Cost Basis (with Fees)
    buy_crypto_tx = transactions[transactions['transaction_type'] == 'buy_crypto_with_usdt'].copy()
    # IMPORTANT: Add fees to the cost
    buy_crypto_tx['total_cost'] = buy_crypto_tx['input_amount'] + buy_crypto_tx.get('fee', 0)
    
    cost_basis_agg = buy_crypto_tx.groupby(['person_name', 'output_currency']).agg(
        total_cost_usd=('total_cost', 'sum'),
    ...
    portfolio_analysis = pd.merge(portfolio, cost_basis[['person_name', 'currency', 'avg_buy_price']], on=['person_name', 'currency'], how='left')
    ...
    # 3. Realized P/L from Sales
    sell_tx = transactions[transactions['transaction_type'] == 'sell'].copy()
    # Merge with cost basis to find the cost of what was sold
    ...
    # Profit = What you got - what it cost you - fees on sale
    ...
    return portfolio_analysis.fillna(0), toman_stats, realized_pnl_summary

# --- Formatting ---
def format_currency(amount, currency):
    if pd.isna(amount): return '-'
    try: num_amount = float(amount)
    except (ValueError, TypeError): return '-'
    if currency == 'IRR': return f"{num_amount:,.0f} Toman"
    return f"${num_amount:,.2f}" # Standardize to USD format for crypto
