...
def update_prices_in_state(symbols, force_refresh=False):
    ...
    unique_symbols = list(set([s for s in symbols if s and pd.notna(s)]))
...
def generate_financial_analysis(transactions, prices):
    ...
    if toman_mask.any():
        ...
        hidden_fee = (tx.loc[toman_mask, 'input_amount'] / rate.replace(0, pd.NA)) - tx.loc[toman_mask, 'output_amount']
    ...
    if transfer_mask.any():
        ...
        price = tx.loc[transfer_mask, 'input_currency'].map(prices).fillna(1.0)
        tx.loc[transfer_mask, 'fee_usd'] = fee_amount * price

    overhead_fees = tx[tx['transaction_type'].isin(['buy_usdt_with_toman', 'transfer'])]
    fee_pool = overhead_fees.groupby('person_name')['fee_usd'].sum().to_dict()
    ...
    if not acquisitions.empty:
        ...
        def allocate_fees(row):
            person, cost = row['person_name'], row['base_cost_usd']
            total_cost = total_acq_cost_per_person.get(person, 0)
