...
تاریخ = st.date_input("تاریخ واریز", value=None)
نوع_ارز_واریزی = st.selectbox("نوع ارز واریزی", ["تومان"] + ارزها)

# تعریف اولیه برای مقدار واریز
مقدار_واریز = ""
مقدار_واریز_پاک = None

# مقدار واریز بر اساس نوع ارز
if نوع_ارز_واریزی == "تومان":
    مقدار_واریز = st.text_input("مقدار واریز (تومان)", value="", placeholder="مثلاً 1,000,000")
    try:
    ...
    مقدار_واریز_پاک = st.number_input("مقدار واریز (دلار)", value=None, min_value=0.0, format="%.2f", step=0.01)
    if مقدار_واریز_پاک:
        مقدار_واریز = f"{مقدار_واریز_پاک:.2f}"
        st.write(f"💵 مقدار وارد شده: {مقدار_واریز} دلار")

else:
    مقدار_واریز_پاک = st.number_input(f"مقدار واریز ({نوع_ارز_واریزی})", value=None, min_value=0.0, format="%.8f", step=0.00000001)
    if مقدار_واریز_پاک:
...
مقدار_تتر = st.number_input("مقدار تتر دریافتی", value=None, min_value=0.0, format="%.4f", step=0.0001)

# نرخ تبدیل تتر به تومان
نرخ_تتر = st.number_input("💱 نرخ تبدیل تتر به تومان", value=None, min_value=0.0, format="%.0f", step=100)
if نرخ_تتر:
    st.write(f"🔹 نرخ وارد شده: {نرخ_تتر:,.0f} تومان")

# محاسبه کارمزد پنهان
if مقدار_واریز_پاک and مقدار_تتر and نرخ_تتر:
    ارزش_واقعی_تتر = مقدار_تتر * نرخ_تتر
    کارمزد_پنهان = مقدار_واریز_پاک - ارزش_واقعی_تتر
    قیمت_تمام_شده_هر_تتر = مقدار_واریز_پاک / مقدار_تتر if مقدار_تتر > 0 else 0
else:
    ارزش_واقعی_تتر = کارمزد_پنهان = قیمت_تمام_شده_هر_تتر = None

# دکمه ثبت
if st.button("ثبت تراکنش"):
    if None in [مقدار_واریز_پاک, مقدار_تتر, نرخ_تتر]:
        st.error("لطفاً همه فیلدها را کامل وارد کنید.")
    else:
