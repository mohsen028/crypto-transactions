...
import pandas as pd
import time
from utils import initialize_state, get_all_transactions, update_prices_in_state, generate_financial_analysis, format_currency
...
portfolio_df, toman_stats_df, realized_pnl_df = generate_financial_analysis(transactions, prices)

# --- Create Tabs ---
tab1, tab2, tab3 = st.tabs(["📊 Floating P/L (Portfolio)", "💰 Realized P/L (Sales)", "🇮🇷 Toman Exchange Stats"])
...
with tab1:
    ...
    if portfolio_df.empty or 'floating_pnl_usd' not in portfolio_df.columns:
        ...
        pnl_summary = portfolio_df.groupby('person_name').agg(
            total_value=('current_value_usd', 'sum'),
            total_cost=('total_cost_of_holdings', 'sum')
        ).reset_index()
...
with tab2:
    ...
    if realized_pnl_df.empty:
        ...
        st.dataframe(
            ...
            column_config={
                "person_name": "Person",
                "realized_pnl": st.column_config.NumberColumn("Net Realized P/L (USD)", format="$%.2f")
            },
...
with tab3:
    ...
    if toman_stats_df.empty:
        st.info("No Toman-to-USDT transactions recorded.")
    else:
        st.dataframe(
            toman_stats_df,
            column_config={
                "person_name": "Person",
                "total_toman_paid": st.column_config.NumberColumn("Total Toman Paid", format="%,.0f Toman"),
                "total_usdt_received": st.column_config.NumberColumn("Total USDT Received", format="%.2f USDT"),
                "avg_usdt_cost": st.column_config.NumberColumn("Avg. Cost per USDT", format="%,.0f Toman")
            },
            hide_index=True, use_container_width=True
        )
